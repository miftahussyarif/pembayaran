...
export async function load() {
	...
	return { dataSmp, santriList, tahunAjarans };
}
...
export const actions = {
	...
	delete: async ({ request }) => {
		...
		try {
			...
			console.error('Error deleting data siswa SMP:', error);
			const msg = String(error?.message || '').toLowerCase();
			if (msg.includes('foreign key') || msg.includes('constraint')) {
				return { success: false, message: 'Tidak bisa menghapus data siswa SMP karena masih terkait dengan data lain.' };
			}
			return { success: false, message: 'Gagal menghapus data siswa SMP.' };
		}
	}
};
